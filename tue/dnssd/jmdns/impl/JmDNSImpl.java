...
import tue.dnssd.context.tag.publish.DNSMessageListener;
import tue.dnssd.jmdns.*;
import tue.dnssd.jmdns.ServiceInfo.Fields;
import tue.dnssd.jmdns.impl.ListenerStatus.ServiceListenerStatus;
import tue.dnssd.jmdns.impl.ListenerStatus.ServiceTypeListenerStatus;
import tue.dnssd.jmdns.impl.constants.DNSConstants;
...
public class JmDNSImpl extends JmDNS implements DNSStatefulObject, DNSTaskStarter {
    ...
    public DNSMessageListener messageListener;
    /**
    ...
    void handleResponse(DNSIncoming msg) throws IOException {
        ...
        if(messageListener!=null)
            messageListener.messageArrived(msg);
    ...
    @Override
    public void publishDNSRecords(List<DNSRecord> records) throws IOException {
        DNSOutgoing outgoing=new DNSOutgoing(0,true);

        for(DNSRecord record: records){
            outgoing.addAnswer(record,new Date().getTime());

            getCache().addDNSEntry(record);
        }
        send(outgoing);
    }
    ...
    @Override
    public void subscribeForRecordUpdates(DNSEntry entry, DNSListener recordUpdatedListener) {

    }

    @Override
    public void unsubscribeForRecordUpdates(DNSEntry entry, DNSListener recordUpdatedListener) {

    }

    @Override
    public void sendmDNSMessage(DNSOutgoing outgoing) throws IOException {
        this.send(outgoing);
    }
