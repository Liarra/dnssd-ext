...
import tue.dnssd.jmdns.impl.JmDNSImpl.ServiceTypeEntry;
...
public class DNSQuestion extends DNSEntry {
    ...
    private static class Pointer extends DNSQuestion {
        ...
        public void addAnswers(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers) {
            ...
            if (this.isServicesDiscoveryMetaQuery()) {
                ...
                Map.Entry<String, List<? extends DNSEntry>> entriesFromCacheMap = cache.getEntry(this.getKey());
                if(entriesFromCacheMap==null) return;

                Collection<? extends DNSEntry> entriesFromCache =entriesFromCacheMap.getValue();
                for (DNSEntry entry : entriesFromCache) {
                    if (entry.getRecordClass() == this.getRecordClass() && entry.getRecordType() == this.getRecordType())
                        if (entry instanceof DNSRecord)
                            answers.add((DNSRecord) entry);
                }
            }
        }
    ...
    protected void addAnswersForServiceInfo(JmDNSImpl jmDNSImpl, Set<DNSRecord> answers, ServiceInfoImpl info) {
        if ((info != null) && info.isAnnounced()) {
            ...
            if(info.hasTags(this.getName())){
                DNSRecord ret=new DNSRecord.Pointer(this.getName(),DNSRecordClass.CLASS_ANY,false,DNSConstants.DNS_TTL,info.getQualifiedName());
                answers.add(ret);
            }
